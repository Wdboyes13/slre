...
static int toi(int x) { return Kisdigit(x) ? x - '0' : x - 'W'; }
...
static int match_op(const unsigned char *re, const unsigned char *s,
    ...
    case '\\':
        ...
        switch (re[1]) {
        case 'S':
            FAIL_IF(Kisspace(*s), SLRE_NO_MATCH);
            result++;
            break;
        case 's':
            FAIL_IF(!Kisspace(*s), SLRE_NO_MATCH);
            ...
            break;
        case 'd':
            FAIL_IF(!Kisdigit(*s), SLRE_NO_MATCH);
            result++;
            break;
        case 'b':
    ...
    while (len <= re_len && re[len] != ']' && result <= 0) {
        ...
        if (re[len] != '-' && re[len + 1] == '-' && re[len + 2] != ']' &&
            ...
            result = info->flags & SLRE_IGNORE_CASE
                         ? Ktolower(*s) >= Ktolower(re[len]) &&
                               Ktolower(*s) <= Ktolower(re[len + 2])
                         : *s >= re[len] && *s <= re[len + 2];
            len += 3;
        } else {
    ...
    for (i = 0; i < re_len; i += step) {
        ...
        if (re[i] == '|') {
            ...
            if (re[i + 1] == 'x') {
                ...
                FAIL_IF(re[i + 1] == 'x' &&
                            !(Kisxdigit(re[i + 2]) && Kisxdigit(re[i + 3])),
                        SLRE_INVALID_METACHARACTER);
            } else {
