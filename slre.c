...
static int is_metacharacter(const unsigned char *s) {
  ...
  return kstrchr(metacharacters, *s) != NULL;
}
...
static int toi(int x) {
  return kisdigit(x) ? x - '0' : x - 'W';
}

static int hextoi(const unsigned char *s) {
  return (toi(ktolower(s[0])) << 4) | toi(ktolower(s[1]));
}
...
static int match_op(const unsigned char *re, const unsigned char *s,
  ...
  switch (*re) {
    case '\\':
      /* Metacharacters */
      switch (re[1]) {
        case 'S': FAIL_IF(kisspace(*s), SLRE_NO_MATCH); result++; break;
        case 's': FAIL_IF(!kisspace(*s), SLRE_NO_MATCH); result++; break;
        case 'd': FAIL_IF(!kisdigit(*s), SLRE_NO_MATCH); result++; break;
    ...
    default:
      if (info->flags & SLRE_IGNORE_CASE) {
        FAIL_IF(ktolower(*re) != ktolower(*s), SLRE_NO_MATCH);
      } else {
  ...
  while (len <= re_len && re[len] != ']' && result <= 0) {
    ...
    if (re[len] != '-' && re[len + 1] == '-' && re[len + 2] != ']' &&
      ...
      result = info->flags &  SLRE_IGNORE_CASE ?
        ktolower(*s) >= ktolower(re[len]) && ktolower(*s) <= ktolower(re[len + 2]) :
        *s >= re[len] && *s <= re[len + 2];
      len += 3;
    } else {
  ...
  for (i = 0; i < re_len; i += step) {
    ...
    if (re[i] == '|') {
      ...
      if (re[i + 1] == 'x') {
        ...
        FAIL_IF(re[i + 1] ==  'x' && !(kisxdigit(re[i + 2]) &&
                kisxdigit(re[i + 3])), SLRE_INVALID_METACHARACTER);
      } else {
...
int slre_match(const char *regexp, const char *s, int s_len,
  ...
  DBG(("========================> [%s] [%.*s]\n", regexp, s_len, s));
  return foo(regexp, (int) kstrlen(regexp), s, s_len, &info);
}
